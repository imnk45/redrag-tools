...
(function (global) {
  ...
  function renderFilters() {
    ...
    if (els.categoryFilters) {
      els.categoryFilters.innerHTML = CONFIG.categories.map(cat =>
        `<button class="rt-btn--filter ${cat === state.category ? 'is-active' : ''}" data-category="${utils.escapeHtml(cat)}">` +
    ...
    if (els.platformFilters) {
      ...
      els.platformFilters.innerHTML = CONFIG.platformFilters.map(pf =>
        `<button class="rt-btn--filter ${pf === state.platformFilter ? 'is-active' : ''}" data-platform="${utils.escapeHtml(pf)}">` +
    ...
    if (els.storageOptions) {
      ...
      els.storageOptions.innerHTML = Object.keys(CONFIG.storage).map(size =>
        `<button class="storage-size-btn ${size === state.storage ? 'is-active' : ''}" data-size="${size}">` +
    ...
    if (els.platformOptions) {
      ...
      els.platformOptions.innerHTML = Object.keys(CONFIG.platforms).map(p =>
        `<button class="storage-platform-btn ${p === state.platform ? 'is-active' : ''}" data-platform="${p}">` +
    ...
    if (els.gameList) {
      els.gameList.innerHTML = filtered.map(game => {
        ...
        return (
          ...
          'data-name="' + utils.escapeHtml(game.name) + '" data-size="' + game.size_gb + '">' +
  ...
  function handleGameListClick(e) {
    // Let checkbox change handler deal with direct checkbox clicks
    ...
    if (els.selectedGamesList) {
      if (selectedGames.length === 0) {
        ...
        els.selectedGamesList.innerHTML = selectedGames.map(g =>
          ...
          '<button class="rt-chip__remove" data-name="' + utils.escapeHtml(g.name) + '" aria-label="Remove ' + utils.escapeHtml(g.name) + '">×</button></div></div>'
        ).join('');
      }
    }
  }
  ...
  function injectSchemas() {
    ...
    seo.breadcrumbSchema([
      { name: 'Home', item: RT.config.urls.blog },
    ...
    // FAQ schema is auto-injected by RT.components.FAQ.init()
  }
  ...
  /* ==========================================
     ...
     ========================================== */
  global.RTTool = {
    meta: { name: 'Game Storage Planner', slug: 'storage', version: '2.0.0' },
    init,
