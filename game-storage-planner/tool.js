...
(function (global) {
  ...
  function init() {
    ...
    loadGames().then(() => {
      ...
      analytics.trackTool('game-storage-planner', 'load');
    }).catch(err => {
  ...
  async function shareResults() {
    ...
    text += '\nPlan your storage at tools.redrag.in/game-storage-planner/';

    const success = await utils.copyToClipboard(text);
    if (success) {
      components.Toast.show({ message: 'Results copied to clipboard!', type: 'success', duration: 3000 });
      analytics.trackTool('game-storage-planner', 'share');
    } else {
  ...
  function resetAll() {
    ...
    analytics.trackTool('game-storage-planner', 'reset');
  }
  ...
  function injectSchemas() {
    const url = 'https://tools.redrag.in/game-storage-planner/';
  ...
  /* ==========================================
     ...
     ========================================== */
  global.RTTool = {
    meta: { name: 'Game Storage Planner', slug: 'game-storage-planner', version: '2.1.0' },
    init,
    _state: state,
    _config: CONFIG
  };
